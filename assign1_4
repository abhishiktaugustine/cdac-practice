...
//in these samples, each application is managed using 
//multiple processes - meaning, multiple processes are 
//used to implement multitasking, in these applications 
//- in this context, each new process is used to 
//do a specific job, for the application 
  ...
  while(i++<5){
   ...
   //this is a child process context and code block 
   if(ret==0) //in a child process context/instance 
   { 
	   printf("I am in child process context\n"); 
        //in a child process context/block, this is the first 
